...
public class CalendarAddBean extends WebControls implements java.io.Serializable {

	private static final long serialVersionUID = 4879601106156013341L;

	private XLogger logger = XLoggerFactory.getXLogger(CalendarAddBean.class.getName());
    ...
    @Value("${calendar}")
    private String sequence_calendar;
	...
	public CalendarAddBean() {
		...
		logger.entry(calendar,day_id,mount_id,year_id);
	}
    ...
    public void addCalendar() {
      ...
      calendar.set(Integer.parseInt(year_id), Integer.parseInt(mount_id) - 1,
          Integer.parseInt(day_id));
      holddate = "" + calendar.getTimeInMillis();
      TransactionStatus status = transactionSupport.beginTransaction();
      String query = "";
      query = sequence_calendar;

      try {
        calendar_id = transactionSupport.getJdbcTemplate().queryForObject(query, String.class);
        query = "insert into calendar (calendar_id , soft_id , site_id , holddate  , active  ) "
            + " values ( " + "" + calendar_id + " , " + "" + product_id + " , " + "" + site_id
            + " , " + "" + holddate + ", " + "true" + " ) ; ";
