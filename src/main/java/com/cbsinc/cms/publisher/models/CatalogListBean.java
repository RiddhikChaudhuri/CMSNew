...
public class CatalogListBean implements java.io.Serializable,ApplicationContext {
	...
	private XLogger logger = XLoggerFactory.getXLogger(CatalogListBean.class.getName());
	...
	public String getTable(String type_id, AuthorizationPageBean authorizationPageBeanId,ServletContext applicationContext) {
		...
		try {
			...
			List<Map<String,Object>> resultList = transactionSupport.getJdbcTemplate().queryForList(query, Long.valueOf(authorizationPageBeanId.getSite_id()),Long.valueOf(authorizationPageBeanId.getCatalog_id()));
			...
			for (int i = 0; resultList.size() > i; i++) {
				rows[i][0] = (String) resultList.get(i).get("catalog_id");
				rows[i][1] = (String)resultList.get(i).get("lable");
	...
	public String getNavigator(AuthorizationPageBean authorizationPageBeanId) {
		...
		query = "select catalog_id ,  lable   FROM catalog WHERE active = true and lang_id = "
				...
				+ " and parent_id = " + authorizationPageBeanId.getCatalog_parent_id() + " limit 50 offset " + offset;

		try {

          List<Map<String,Object>> resultList = transactionSupport.getJdbcTemplate().queryForList(query);
	...
	public boolean isFolder(String parent_id) {
		...
		try {
			...
			count = Long.parseLong(transactionSupport.getJdbcTemplate().queryForObject(query,String.class));
		...
		try {
			...
			while (doWhile) {
				...
				if (resultList.size() > 0) {
					if (((String) resultList.get(0).get("parent_id")).length() > 0)
						_parent_id = Long.parseLong((String)resultList.get(0).get("parent_id"));
					lable = (String)resultList.get(0).get("lable");
	...
	public String getCatalogUrlPath(AuthorizationPageBean authorizationPageBeanId) {
		...
		StringBuffer path = new StringBuffer();
		if (authorizationPageBeanId.getCatalog_id().length() > 0)
			_parent_id = Long.parseLong(authorizationPageBeanId.getCatalog_parent_id());
		if (_parent_id == 0)
			return lable_last;
		String item = "";
		...
		try {
			...
			while (doWhile) {
				...
				if (resultList.size() > 0) {
					if (((String) resultList.get(0).get("parent_id")).length() > 0)
						parent_id_last = Long.parseLong((String) resultList.get(0).get("parent_id"));
					lable = (String) resultList.get(0).get("lable");
					doWhile = true;
	...
	public String getCatalogXMLUrlPath(String pagejsp, String name, String selected_cd,
		...
		try {
			...
			while (doWhile) {
				// Adp.BeginTransaction();
				query = "select parent_id , lable   FROM catalog WHERE  catalog_id  = " + _parent_id;
				List<Map<String,Object>> listForCatalog = jdbcTemplate.queryForList(query);
				if (listForCatalog.size() > 0) {
					if (((String)listForCatalog.get(0).get("parent_id")).length() > 0)
						parent_id_last = Long.parseLong((String)listForCatalog.get(0).get("parent_id"));
					lable = (String)listForCatalog.get(0).get("lable");
					doWhile = true;
	...
	public String getCatalogXMLUrlPath(String pagejsp, String name, String selected_cd, String catalog_id,
		...
		try {
			...
			while (doWhile) {
				query = "select parent_id , lable   FROM catalog WHERE  catalog_id  = " + _parent_id;
				List<Map<String,Object>> catalogIDList =  transactionSupport.getJdbcTemplate().queryForList(query);
				if (catalogIDList.size() > 0) {
					if (((String) catalogIDList.get(0).get("parent_id")).length() > 0)
						parent_id_last = Long.parseLong((String) catalogIDList.get(0).get("parent_id"));
					lable = (String) catalogIDList.get(0).get("lable");
					doWhile = true;
					// item = "<a href=\"ProductPostCre.jsp?parent_id=" + _parent_id +"\" >"+"/" +
					// lable + "</a>" ;
					rowItem = new StringBuffer();
	...
	public Boolean getIsWriteble() {
		return isWriteble;
	}

	public void setIsWriteble(Boolean isWriteble) {
		this.isWriteble = isWriteble;
	}
