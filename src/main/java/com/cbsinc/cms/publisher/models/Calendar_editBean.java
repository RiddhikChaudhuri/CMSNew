 ...
 */
@Component
public class Calendar_editBean extends WebControls implements
		java.io.Serializable {
	
	private static final long serialVersionUID = 8381891800819464179L;

	//static private Logger log = Logger.getLogger(Calendar_editBean.class);
	 private XLogger log = XLoggerFactory.getXLogger(Calendar_editBean.class.getName());
	...
	public void editCalendar() {
		...
		query = "update calendar set soft_id = ? holddate = ? where calendar_id = ?";

		try 
		{
		  transactionSupport.getJdbcTemplate().update(query, product_id, holddate,  calendar_id);
