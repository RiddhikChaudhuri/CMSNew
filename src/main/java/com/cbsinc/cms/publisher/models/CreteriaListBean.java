...
public class CreteriaListBean implements java.io.Serializable {
	...
	private XLogger log = XLoggerFactory.getXLogger(CreteriaAddBean.class.getName());
	...
	public void initPage(String addstring) {
		...
		try {
			List<Map<String,Object>> list = transactionSupport.getJdbcTemplate().queryForList(query);
			listDataGet.addList(list);

			if (listDataGet.getRowCount() > 0)
				title = (String)list.get(0).get("label");
		} catch (Exception ex) {
