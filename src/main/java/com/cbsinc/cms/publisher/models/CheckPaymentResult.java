...
public class CheckPaymentResult implements java.io.Serializable {
	...
	private XLogger logger = XLoggerFactory.getXLogger(CheckPaymentResult.class.getName());
	...
	@Autowired
    public CheckPaymentResult(@Value("${checkpay_template}")String checkpay_template) {
	...
	public OrderBank getOrder() {
		...
		if (rows.size() > 0) {
			try {
				...
				objOrderBank.setBeginData((String)rows.get(0).get("date_input"));
			} catch (Exception ex) {
    ...
    public void parserRequest(String i_strNumerOrder, String i_strRezult, String i_strDecsription) {
      logger
          .entry("Order: " + i_strNumerOrder + " Rezalt: " + i_strRezult + " " + i_strDecsription);
