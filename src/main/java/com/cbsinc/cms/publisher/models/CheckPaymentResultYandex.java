...
public class CheckPaymentResultYandex implements java.io.Serializable {

	transient private static final long serialVersionUID = 8160271389486247546L;
	...
	private XLogger logger = XLoggerFactory.getXLogger(CheckPaymentResultYandex.class.getName());

	private URL url1;
	transient ResourceBundle setup_resources = null;
	long delay = 60000;
	
    private final String checkpay_yandex;
	String strUrlServer = "http://pgate.grabko.com:88";

	   @Autowired
	    private TransactionSupport transactionSupport;

	   
	public CheckPaymentResultYandex(@Value("${checkpay.yandex}")String checkpay_yandex) {
	    this.checkpay_yandex=checkpay_yandex;
		delay = Long.parseLong(checkpay_yandex.trim());
		logger.entry(delay);
		try {
	...
	transient java.util.TimerTask t = new java.util.TimerTask() {
		public void run() {
			...
			if (objOrderBank != null && objOrderBank.getOrder_ID() != null) {
				...
				logger.info("Send  request to " + strUrlServer + " pay gateway: "
	...
	public OrderBank getOrder() {
      ...
      try {
        query =
            "SELECT id, date_input FROM account_hist WHERE complete = false and active = true ORDER BY id ASC LIMIT 1";
	...
	void end_addmoney(TransactionStatus status, String i_strAccountHistory_id, String i_strRezult, String i_strDecsription)
			throws Exception {
