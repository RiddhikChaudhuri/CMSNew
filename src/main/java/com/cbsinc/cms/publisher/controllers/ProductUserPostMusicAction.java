...
@RestController
public class ProductUserPostMusicAction extends CMSObjects {

  private XLogger logger = XLoggerFactory.getXLogger(ProductUserPostMusicAction.class.getName());
    ...
    @PostMapping(value="/doPostProductUserMusic",consumes = MediaType.APPLICATION_JSON_VALUE,produces = MediaType.APPLICATION_JSON_VALUE)
    @ApiOperation(value = "Post For Product User Music Controller",notes = "Returns Product User Music Controller based on the Request")
    ...
    @GetMapping(value="/doGetProductUserMusic",consumes = MediaType.APPLICATION_JSON_VALUE,produces = MediaType.APPLICATION_JSON_VALUE)
    @ApiOperation(value = "Get For Product User Music Controller",notes = "Returns Product User Music Controller based on the Product ID")
	public @ResponseBody CMSNewBlockPostCreResponseModel doGet(@RequestBody ProductPostCreForm productListWebForm) throws Exception 
	{
	    action(productListWebForm) ;
	...
	public void action(ProductPostCreForm productListWebForm) throws Exception {
      ...
      productlistBeanId = getProductListBean().get();
      ...
      // if( resources == null ) resources = PropertyResourceBundle.getBundle("localization",
      // response.getLocale());
      if (notselected.length() == 0)
        notselected =
            authorizationPageBeanId.getLocalization(getServletContext()).getString("notselected");
...
//		 End Novigator ---
		...
		if(productListWebForm.getType_id() != null){ publisherBeanId.setType_id( productListWebForm.getType_id()) ; }
		...
		if(productListWebForm.getSalelogic_id() !=null ) publisherBeanId.setProgname_id( productListWebForm.getSalelogic_id());
